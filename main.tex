...
\newcommand{\facilityphoto}[3]{%
\IfFileExists{#1}{%
\includegraphics[width=\linewidth,height=#2,keepaspectratio]{#1}
}{%
\begin{center}
\fbox{%
\begin{minipage}[c][#2][c]{0.92\linewidth}
\centering
\textbf{#3}\\[0.2cm]
...
\subtitle{\includegraphics[height=2.9ex]{images/pngegg.png} TÜBİTAK 1507 -- KOBİ Ar-Ge Başlangıç Destek Programı}
...
\begin{itemize}
\item Yüksek matematik (eliptik kuaterniyon cebiri, matris teorisi)
\item Görüntü işleme ve derin öğrenme algoritmaları
\item Kriptografi ve dijital damgalama teknolojileri
...
\begin{frame}{Kuruluşun Altyapısının Uygunluğu}
\bglogo
...
\column{0.5\textwidth}
\textbf{Proje ile Uyum}
\begin{itemize}
\item Proje çıktısı fiziksel üretim değil, yazılım/algoritma çekirdeğidir.
\item Mevcut SaaS/API altyapısı öz kaynaklarla geliştirilmiş durumdadır.
\item EQCore geliştirme, test ve entegrasyon süreçleri ofis tabanlı yürütülebilir.
\end{itemize}
\end{columns}
\end{frame}
...
\begin{columns}
\column{0.5\textwidth}
\begin{block}{Sakarya Teknokent Binası}
\facilityphoto{images/sakarya_teknokent_bina.png}{4.2cm}{Sakarya Teknokent Binası}
\end{block}

\column{0.5\textwidth}
\begin{block}{QuatVision Ofisi}
\facilityphoto{images/quatvision_ofis.png}{4.2cm}{QuatVision Ofisi}
\end{block}
\end{columns}

\vspace{0.2cm}
\begin{alertblock}{Altyapı Uygunluğu}
45 m$^2$ ofis alanı; proje personelinin yazılım geliştirme, matematiksel modelleme, deneysel test, raporlama ve mevcut QuatMark SaaS/API altyapısına entegrasyon çalışmalarını yürütmesi için yeterlidir.
\end{alertblock}
\end{frame}
...
\begin{block}{Yürütme Yetkinliği}
\begin{itemize}
\item Bilgisayar görüsü, görüntü işleme ve dijital veri güvenliği yetkinliği
\item Kriptografi, dijital damgalama ve eliptik kuaterniyon matematiği odağı
\item Teknokent ortamında Ar-Ge, test, dokümantasyon ve pilot doğrulama süreçlerini sürdürebilecek organizasyon yapısı
\end{itemize}
\end{block}
\end{columns}
...
\vspace{0.2cm}
\begin{center}
\textbf{Sonuç:} Kuruluşun fiziksel altyapısı, mevcut yazılım ürünü ve Ar-Ge yetkinlikleri, QuatMark-EQCore projesinin planlanan iş paketlerini yürütmek için uygundur.
\end{center}
\end{frame}
...
\item Tek katmanlı güvenlik yapısı
\item Derin öğrenme tabanlı saldırılara karşı sınırlı dayanıklılık
\end{itemize}
\end{alertblock}
...
\item \textbf{Güvenlik:} Arnold karıştırma + kaotik kriptografi + \textbf{SHA-2} tabanlı pseudo-rastgele konum seçimi $\rightarrow$ \textbf{çok katmanlı} mimari (rakiplerde kamuya açık DL savunması yok).
...
$N \times N$ boyutlarındaki bir görüntüde yer alan $(x, y)$ koordinatındaki bir pikselin, bir sonraki iterasyondaki yeni $(x', y')$ konumu:
$$
